...
// Nous n'avons plus besoin de createClient() directement ici
// La récupération des couleurs se fera via l'API Route.
...
function SellModal({ color, onClose, onListingSuccess }: { color: OwnedColor, onClose: () => void, onListingSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || "Une erreur est survenue lors de la mise en vente.");
      }
      onListingSuccess();
      onClose();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
      <div className="bg-[#181818] p-8 rounded-lg w-full max-w-md animate-fade-in">
        ...
        <form onSubmit={handleSubmit}>
          <label htmlFor="price-input" className="block mb-2 font-medium">Prix de vente (EUR)</label>
          <input 
            id="price-input"
...
export default function DashboardPage() {
  ...
  // Correction : fetchColors appelle l'API Route
  const fetchColors = useCallback(async () => {
    setIsLoadingColors(true);
    try {
      const response = await fetch('/api/user/colors'); // Appel de l'API Route
      if (!response.ok) {
        throw new Error('Erreur lors de la récupération des couleurs du Gardien.');
      }
      const data: OwnedColor[] = await response.json();
      setColors(data);
    } catch (error) {
      console.error("Erreur de fetchColors:", error);
      setColors([]);
    } finally {
  ...
  if (status === 'loading' || !session) {
    return <div className="flex items-center justify-center min-h-screen text-neutral-300">Vérification de la session...</div>;
  }
  ...
  return (
    <>
      <div className="container mx-auto p-4 md:p-8">
        ...
        {isLoadingColors ? (
          <div className="text-center py-20 text-neutral-300">Chargement de vos couleurs...</div>
        ) : colors.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-xl text-neutral-500">Vous ne possédez aucune couleur pour le moment.</p>
            <Link href="/marketplace" className="mt-4 inline-block text-purple-400 hover:text-purple-300">
              Explorer le Marché
            </Link>
          </div>
        ) : (
              ...
              >
                ...
                {!color.is_for_sale ? (
                  <button 
                    ...
                    className="absolute top-2 right-2 bg-green-500 text-white text-xs font-bold px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity z-20">
                    Vendre
                  </button>
                ) : (
                  <div className="absolute top-2 right-2 bg-yellow-500 text-black text-xs font-bold px-2 py-1 rounded z-20">
                    En vente
                  </div>
                )}
              </div>
